...
class GlycanLinkDataset(torch_data.Dataset, core.Configurable):
    ...
    Parameters:
        ...
        **kwargs
    """
    url = "https://torchglycan.s3.us-east-2.amazonaws.com/downstream/glycan_link.csv"
    ...
    def __init__(self, path, view, target_fields=None, verbose=1, **kwargs):
        ...
        self.path = path
        self.csv_file = os.path.join(path, "glycan_link.csv")
        ...
        self.view = view
        if view == "atom":
            self.csv_with_smiles_file = os.path.join(path, "glycan_link_with_smiles.csv")
            if not os.path.exists(self.csv_with_smiles_file) and view == "atom":
                self.generate_csv_with_mol(verbose=verbose, **kwargs)
            self.load_csv(self.csv_with_smiles_file, iupac_field="glycan", target_fields=self.target_fields,
                        verbose=verbose, **kwargs)
        elif view == "glycan" or view == "atom-glycan" or "bi":
            self.load_csv(self.csv_file, iupac_field="glycan", target_fields=self.target_fields,
                        verbose=verbose, **kwargs)
        else:
    ...
    def load_iupac(self, iupac_list, smiles_list, targets, sample_splits, transform=None, lazy=False, verbose=0, **kwargs):
        """
    ...
    def generate_csv_with_mol(self, verbose=0, **kwargs):
        print("Construct SMILES by GlyLES and saved in glycan_link_with_smiles.csv")
