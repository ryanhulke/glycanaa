...
class GlycanImmunogenicityDataset(torch_data.Dataset, core.Configurable):
    ...
    def __init__(self, path, view, target_fields=None, verbose=1, **kwargs):
        ...
        self.csv_file = os.path.join(path, "glycan_immunogenicity.csv")
        if not os.path.exists(self.csv_file):
            self.csv_file = utils.download(self.url, path, md5=self.md5)

        self.view = view
        if view == "atom":
            self.csv_with_smiles_file = os.path.join(path, "glycan_immunogenicity_with_smiles.csv")
    ...
    def generate_csv_with_mol(self, verbose=0, **kwargs):
        logger.warning("Construct SMILES by GlyLES and saved in glycan_immunogenicity_with_smiles.csv")
