...
from module.metric import macro_f1
...
class GlycanPropertyPrediction(tasks.PropertyPrediction):
    ...
    Parameters:
        model2 (nn.Module, optional): use model2 instead of model if specified.
    ...
    def __init__(self, model2=None, adjust="default", **kwargs):
        
        if "model" in kwargs and model2:
            kwargs["model"] = model2
    ...
    def _evaluate(self, pred, target):
        labeled = ~torch.isnan(target)
        ...
        for _metric in self.metric:
            if _metric == "mae":
                ...
                for i, cur_num_class in enumerate(self.num_class):
                    ...
                    _target = target[:, i]
                    _labeled = labeled[:, i]
                    _score = macro_f1(_pred[_labeled], _target[_labeled].long())
                    score.append(_score)
                    num_class += cur_num_class
                score = torch.stack(score)
            elif _metric == "mcc":
    ...
    def evaluate(self, pred, target):
        metric = self._evaluate(pred, target)
        for _metric in self.metric:
            name = tasks._get_metric_name(_metric)
